...
Shared libraries built by a `build.lua` script are automatically included. See [C Module Support](/docs/general/misc/c-module-support) for how to produce them.
...
- [Analyzing Binary Bloat](/docs/bundler/features/bloat) — see what makes up the compiled binary, per dependency and per file.
